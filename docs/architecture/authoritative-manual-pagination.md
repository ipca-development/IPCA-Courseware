...
- During the Phase D paragraph proof, transparent projection portals are positioned from stored authoritative block measurements. A portal only focuses the matching canonical source paragraph and restores a semantic source caret; the projected iframe remains sandboxed, non-editable, and excluded from save payloads and undo history.
