...
$enabled = (string)$pdo->query("SELECT config_value FROM ipca_communication_app_config WHERE config_key = 'community_enabled'")->fetchColumn();
$posting = (string)$pdo->query("SELECT config_value FROM ipca_communication_app_config WHERE config_key = 'community_posting_enabled'")->fetchColumn();
$posts = (int)$pdo->query("SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ipca_community_posts'")->fetchColumn();
echo 'community_enabled=' . $enabled . PHP_EOL;
echo 'community_posting_enabled=' . $posting . PHP_EOL;
echo 'community_posts_table=' . ($posts > 0 ? 'yes' : 'no') . PHP_EOL;
echo 'ok' . PHP_EOL;
