...
$options = getopt('', array('version-ids:', 'actor-id:', 'apply'));
...
if ($ids === array() || $actorId <= 0) {
    fwrite(STDERR, "Usage: php scripts/refresh_released_publication_packages.php "
        . "--version-ids=1,2 --actor-id=25 [--apply]\n");
    exit(1);
}
...
foreach ($ids as $versionId) {
    ...
    if (!is_array($version) || (string)($version['lifecycle_status'] ?? '') !== 'released') {
        throw new RuntimeException("Version {$versionId} is not released.");
    }
    $approval = $reader->pageMapStore()->approvalMeta($versionId);
    if (!is_array($approval) || (string)($approval['status'] ?? '') !== 'approved') {
        throw new RuntimeException("Version {$versionId} has no approved page map.");
    }
    ...
    echo sprintf(
        "%d %s style=%s manifest=%s snapshot=%s mode=%s\n",
        $versionId,
