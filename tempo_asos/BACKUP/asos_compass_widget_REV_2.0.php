...
(function(){
 ...
 function pmMiniSvg(wd, ws){
  // If direction is missing even after carry-forward, show placeholder
  ...
  // 180 reversed (wind FROM -> arrow points INTO wind)
...
for (var i=0;i<points.length;i++){
  ...
  // ? Carry forward previous valid direction
  if (wd === null || isNaN(parseInt(wd,10))) {
    wd = lastValidWd;
  } else {
    wd = parseInt(wd,10);
    lastValidWd = wd;
  ...
  // ? Carry forward previous valid speed
  if (ws === null || isNaN(parseInt(ws,10))) {
    ws = lastValidWs;
  } else {
    ws = parseInt(ws,10);
    lastValidWs = ws;
