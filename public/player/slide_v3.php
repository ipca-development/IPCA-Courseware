...
<head>
  ...
  <title>Slide (v3 · Maya Coach)</title>
...
document.getElementById('btnPrev').onclick = async ()=>{
  ...
  armAutoplay();
  location.href = '/player/slide_v3.php?slide_id=' + PREV_ID;
};	
...
document.getElementById('btnNext').onclick = async ()=>{
  ...
  armAutoplay();
  location.href = '/player/slide_v3.php?slide_id=' + NEXT_ID;
};
 ...
 * =====================================================================
 *
 * The coach is mounted inside the drawer next to / below the existing
 * editor. It calls /student/api/summary_coach.php for all AI work.
 * When Maya runs final_review and approves, the API delegates canonical
 * acceptance to LessonSummaryService::checkSummary() server-side. After
 * that, this page refreshes the production-side status banner using the
 * existing helper (refreshSummaryStatusOnly + renderSummaryAlert) so the
 * student sees the same canonical status they would get from the
 * production "Check my Summary" path.
 */
(function () {
  ...
  // flushing the production save also keeps the canonical row in sync.
  function flushDraftQuiet() {
    try {
      if (!isLocked && drawer.style.display === 'flex') {
        flushSummarySaveNow();
      }
    } catch (e) {}
  }
    ...
    onCanonicalAccept: function (canonicalCheck /*, mayaResponse */) {
      ...
      try {
        if (canonicalCheck && canonicalCheck.ok && canonicalCheck.review_status === 'acceptable') {
          isLocked = Number(canonicalCheck.student_soft_locked || 0) === 1;
          currentReviewStatus = String(canonicalCheck.review_status || 'acceptable');
          updateDrawerLockState();
          renderSummaryAlert({
            review_status: currentReviewStatus,
            review_feedback: String(canonicalCheck.review_feedback || ''),
            review_notes_by_instructor: ''
          }, { forceShow: true });
        } else {
          // Fall back to a fresh server fetch in case the canonical row was
          // updated despite a partial response shape.
          refreshSummaryStatusOnly();
        }
      } catch (e) {
  ...
  function bootMaya() {
    if (!window.IPCASummaryCoach) return;
    ...
    // baseline length is fresh).
    var origSummaryClick = document.getElementById('btnSummary').onclick;
    document.getElementById('btnSummary').onclick = function () {
      if (typeof origSummaryClick === 'function') origSummaryClick();
      if (coach && typeof coach.refreshEditor === 'function') {
        coach.refreshEditor();
      }
      syncMayaModeWithDrawer();
    };

    // When student clicks Continue with Maya, also flush the in-flight draft
    // so the server-side has the latest plain-text excerpt.
    var btnCont = document.querySelector('#mayaCoachRoot [data-maya-continue]');
    if (btnCont) {
      btnCont.addEventListener('click', flushDraftQuiet, true);
    }
    var btnFinal = document.querySelector('#mayaCoachRoot [data-maya-final]');
    if (btnFinal) {
      btnFinal.addEventListener('click', flushDraftQuiet, true);
    }
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bootMaya);
  } else {
    bootMaya();
  }
})();
