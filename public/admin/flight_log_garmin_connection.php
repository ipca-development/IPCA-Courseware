...
function garmin_sync_status_label(array $summary, string $classification): string
{
    ...
    $hasCounters = (string)($summary['hobbs_out'] ?? '--') !== '--' && (string)($summary['tacho_out'] ?? '--') !== '--';
...
<div class="garmin-page">
  ...
  <section class="garmin-card">
    ...
    <form class="garmin-filter" method="get" action="/admin/flight_log_garmin_connection.php">
      <div class="garmin-filter-row">
        <label><span class="garmin-label">Tail</span><select class="garmin-filter-short" name="tail" data-filter-field="tail"><option value="">All</option><?php foreach ($tailOptions as $tailOption): ?><option value="<?= h($tailOption) ?>"<?= $filterTail === $tailOption ? ' selected' : '' ?>><?= h($tailOption) ?></option><?php endforeach; ?></select></label>
        <label><span class="garmin-label">From</span><input class="garmin-filter-date" type="date" name="date_from" value="<?= h($filterDateFrom) ?>"></label>
        <label><span class="garmin-label">To</span><input class="garmin-filter-date" type="date" name="date_to" value="<?= h($filterDateTo) ?>"></label>
        <label><span class="garmin-label">Dep AD</span><select class="garmin-filter-short" name="dep" data-filter-field="dep"><option value="">All</option><?php foreach ($depOptions as $airportOption): ?><option value="<?= h($airportOption) ?>"<?= $filterDep === $airportOption ? ' selected' : '' ?>><?= h($airportOption) ?></option><?php endforeach; ?></select></label>
        <label><span class="garmin-label">Arr AD</span><select class="garmin-filter-short" name="arr" data-filter-field="arr"><option value="">All</option><?php foreach ($arrOptions as $airportOption): ?><option value="<?= h($airportOption) ?>"<?= $filterArr === $airportOption ? ' selected' : '' ?>><?= h($airportOption) ?></option><?php endforeach; ?></select></label>
      </div>
      <div class="garmin-filter-row">
        <label><span class="garmin-label">Status</span><select class="garmin-filter-medium" name="status"><option value="">Any</option><?php foreach (array('Complete','Partial','GPS only','Needs review') as $option): ?><option value="<?= h($option) ?>"<?= $filterStatus === $option ? ' selected' : '' ?>><?= h($option) ?></option><?php endforeach; ?></select></label>
        <label><span class="garmin-label">Visibility</span><select class="garmin-filter-medium" name="show_incomplete"><option value="0">Complete only</option><option value="1"<?= $showIncomplete ? ' selected' : '' ?>>Show incomplete</option></select></label>
        <label><span class="garmin-label">Hidden</span><select class="garmin-filter-medium" name="show_hidden"><option value="0">Hide hidden</option><option value="1"<?= $showHidden ? ' selected' : '' ?>>Show hidden</option></select></label>
        <label><span class="garmin-label">New</span><select class="garmin-filter-short" name="new_only"><option value="0">All</option><option value="1"<?= $newOnly ? ' selected' : '' ?>>New only</option></select></label>
        <div><span class="garmin-label">&nbsp;</span><button class="secondary garmin-filter-short" type="submit">Apply</button></div>
      </div>
    </form>
