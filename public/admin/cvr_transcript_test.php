...
function cvrt_env_first(array $keys, string $default = ''): string
{
    foreach ($keys as $key) {
        $value = getenv($key);
...
function cvrt_parse_bucket_from_cdn(?string $cdnBase): string
{
    $cdnBase = trim((string)$cdnBase);
    if ($cdnBase === '') {
        return '';
    }

    $host = (string)parse_url($cdnBase, PHP_URL_HOST);
    if ($host === '') {
        return '';
    }
...
function cvrt_spaces_config(): array
{
    global $CDN_BASE;

    $cdnBase = (string)($CDN_BASE ?? cvrt_env_first(['CW_CDN_BASE'], ''));

    $bucket = cvrt_env_first([
        'CW_DO_SPACES_BUCKET',
        'DO_SPACES_BUCKET',
        'SPACES_BUCKET',
        'AWS_BUCKET',
        'S3_BUCKET',
    ], cvrt_parse_bucket_from_cdn($cdnBase));

    $region = cvrt_env_first([
        'CW_DO_SPACES_REGION',
        'DO_SPACES_REGION',
        'SPACES_REGION',
        'AWS_DEFAULT_REGION',
    ...
    $endpoint = cvrt_env_first([
        'CW_DO_SPACES_ENDPOINT',
        'DO_SPACES_ENDPOINT',
        'SPACES_ENDPOINT',
        'AWS_ENDPOINT',
        'S3_ENDPOINT',
    ...
    return [
        ...
        'cdn_base' => rtrim($cdnBase, '/'),
    ];
}

function cvrt_spaces_is_ready(array $cfg): bool
{
    return $cfg['bucket'] !== '' && $cfg['region'] !== '' && $cfg['key'] !== '' && $cfg['secret'] !== '';
}
...
function cvrt_s3_request(array $cfg, string $method, string $objectKey = '', array $query = []): array
{
    $times = cvrt_amz_now();
...
function cvrt_list_input_files(array $cfg, string $prefix): array
{
    $resp = cvrt_s3_request($cfg, 'GET', '', [
        'list-type' => '2',
        'prefix'    => $prefix,
        'max-keys'  => '200',
    ]);
...
function cvrt_download_to_temp(array $cfg, string $objectKey): string
{
    $resp = cvrt_s3_request($cfg, 'GET', $objectKey);
...
try {
    if (!cvrt_spaces_is_ready($spacesCfg)) {
        throw new RuntimeException(
            'Spaces configuration is not complete. Required env values were not detected. ' .
            'This page checks common names automatically, including bucket, region, key, and secret.'
        );
    }

    $files = cvrt_list_input_files($spacesCfg, $spacesPrefix);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $tmpFile = cvrt_download_to_temp($spacesCfg, $selectedKey);
...
<div class="cvrt-stack">
  ...
  <div class="card cvrt-hero">
    ...
    <div class="cvrt-sub">
      Upload audio test files to <strong><?= h($spacesPrefix) ?></strong> inside the <strong><?= h((string)$spacesCfg['bucket']) ?></strong> Space, pick the file here, tweak the transcription prompt, and review the output as ATC, intercom, and probable noise.
    </div>
  </div>

  <?php if ($error !== ''): ?>
    <div class="cvrt-alert error"><?= h($error) ?></div>
  <?php endif; ?>

  <?php if ($success !== ''): ?>
    <div class="cvrt-alert success"><?= h($success) ?></div>
  <?php endif; ?>
    ...
    <div class="card cvrt-card">
      ...
      <form method="post" action="">
        <div class="cvrt-field">
          ...
          <select class="cvrt-select" id="selected_key" name="selected_key">
            ...
            <?php foreach ($files as $file): ?>
              <option value="<?= h((string)$file['key']) ?>" <?= ((string)$file['key'] === $selectedKey ? 'selected' : '') ?>>
                <?= h((string)$file['basename']) ?> — <?= h(cvrt_file_size_label((int)$file['size_bytes'])) ?>
              </option>
            <?php endforeach; ?>
          </select>
        </div>
        ...
        <div class="cvrt-field">
          ...
          <textarea class="cvrt-textarea" id="prompt" name="prompt"><?= h($prompt) ?></textarea>
        </div>
    ...
    <div class="card cvrt-card">
      <div class="cvrt-card-head">
        <div>
          ...
          <div class="cvrt-card-sub">Current source folder and detected environment information for this test page.</div>
        </div>
      ...
      <div class="cvrt-meta-grid">
        <div class="cvrt-metric">
          ...
          <div class="cvrt-metric-value"><?= h((string)$spacesCfg['bucket']) ?></div>
        </div>
        <div class="cvrt-metric">
          <div class="cvrt-metric-label">Region</div>
          <div class="cvrt-metric-value"><?= h((string)$spacesCfg['region']) ?></div>
        </div>
        <div class="cvrt-metric">
          <div class="cvrt-metric-label">Prefix</div>
          <div class="cvrt-metric-value"><?= h($spacesPrefix) ?></div>
        </div>
        <div class="cvrt-metric">
      ...
      <?php if ($selectedMeta): ?>
        <div class="cvrt-empty" style="margin-top:16px;">
          <strong>Selected:</strong> <?= h((string)$selectedMeta['basename']) ?><br>
          <strong>Object key:</strong> <?= h((string)$selectedMeta['key']) ?><br>
          <strong>Size:</strong> <?= h(cvrt_file_size_label((int)$selectedMeta['size_bytes'])) ?><br>
          <strong>Updated:</strong> <?= h((string)$selectedMeta['last_modified']) ?>
        </div>
      <?php else: ?>
        <div class="cvrt-empty" style="margin-top:16px;">
          Drop test audio files into the Spaces folder <strong><?= h($spacesPrefix) ?></strong> and they will appear in the selector above.
        </div>
      <?php endif; ?>
    </div>
  </div>
    ...
    <?php else: ?>
      <div class="cvrt-transcript-wrap">
        <?php foreach ($segments as $seg): ?>
          ...
          ?>
          <div class="cvrt-row <?= h($rowClass) ?>">
            <div class="cvrt-time"><?= h($timeLabel) ?></div>
            <div class="cvrt-bubble-lane">
              <div class="cvrt-bubble <?= h($rowClass) ?>">
                <div class="cvrt-bubble-head">
                  <div class="cvrt-actor <?= h($rowClass) ?>"><?= h($actor) ?></div>
                </div>
                <div class="cvrt-bubble-text"><?= h((string)($seg['text'] ?? '')) ?></div>
              </div>
            </div>
          </div>
        <?php endforeach; ?>
      ...
      <?php if ($transcriptionRaw !== null): ?>
        <details class="cvrt-debug">
          ...
          <div class="cvrt-json"><?= h(json_encode($transcriptionRaw, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)) ?></div>
        </details>
      <?php endif; ?>
    <?php endif; ?>
  </div>
