...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($labId <= 0 || $courseTitle === '' || !$lessonIds) {
        progress("ERROR: Lab ID, Course Title, and Lesson IDs JSON are required.");
    ...
    ");
    $stmt->execute([$programId, $courseTitle, $courseSlug, '1.0', $courseOrder, 0, $labId]);

    $stmt = $pdo->prepare("SELECT id FROM courses WHERE program_id=? AND slug=? LIMIT 1");
    $stmt->execute([$programId, $courseSlug]);
    ...
    foreach ($lessonIds as $extLessonId) {
        ...
        $insLesson->execute([$courseId, $extLessonId, "Lesson {$extLessonId}", $sort, $pageCount, $defaultTpl]);
    ...
    progress("DONE. Imported Lab {$labId}. Total slides generated: {$totalSlides}");
    echo "<p><a href='/admin/courses.php'>Courses</a> | <a href='/admin/lessons.php'>Lessons</a> | <a href='/admin/slides.php'>Slides</a> | <a href='/admin/import_lab.php'>Import another lab</a></p>";
    echo "</body></html>";
    exit;
}
...
<div class="card">
  <p class="muted">
    Import one Lab at a time. This tool will create Course → Lessons → Slides and auto-detect page_count by probing your CDN.
  </p>
  ...
  <form method="post" class="form-grid">
    ...
    <label>Lab ID</label>
    <input name="lab_id" type="number" required>

    <label>Course title</label>
    <input name="course_title" required placeholder="e.g. Airspace">
    ...
    <textarea name="lesson_ids_json" rows="10" style="width:100%; grid-column: 2 / 3;"
      placeholder='Paste either: {"labId":10013,"lessonIds":[10152,10153,...]} OR just [10152,10153,...]'></textarea>

    <div></div>
    <button class="btn" type="submit">Import Lab</button>
  </form>
</div>
<?php cw_footer(); ?>
