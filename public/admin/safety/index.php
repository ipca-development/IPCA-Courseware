...
(() => {
  ...
  async function reportDetail(uuid) {
    ...
    const flight=r.flight_link, flightContext=flight?.context||{}, recordings=flight?.recordings||[], garmin=flight?.garmin_csv_files||[];
    const flightCard=flight?`<div class="sms-card"><h4>Linked flight evidence</h4><div><strong>${esc(flightContext.aircraft_registration||'Aircraft pending')} · ${esc(flightContext.mission_code||flightContext.mission_name||'Mission not set')}</strong> ${pill(flight.resolution_state)}</div><div class="sms-sub">${esc(flightContext.departure_airport||'')} ${flightContext.destination_airport?'→ '+esc(flightContext.destination_airport):''} · Schedule #${esc(flight.schedule_slot_id||'—')}</div><div class="sms-sub" style="margin-top:8px">CVR recording(s): ${recordings.length} · Published transcript(s): ${recordings.filter(x=>x.published_transcript_version_id).length} · Garmin/CSV file(s): ${garmin.length} · Replay source: ${recordings.length&&garmin.length?'available or processing':'pending'}</div></div>`:'';
    ...
    <div class="sms-detail-grid" style="margin-top:16px"><div class="sms-stack">
      ...
      <div class="sms-card"><h4>Reporter observations</h4><div class="sms-sub">Phase: ${esc(r.phase_of_flight||'Not supplied')} · Injury: ${esc(label(r.injury_state))} · Damage: ${esc(label(r.damage_state))} · Weather: ${esc(label(r.weather_relevance))}</div>${r.immediate_action?`<div class="sms-narrative" style="margin-top:8px">${esc(r.immediate_action)}</div>`:''}</div>
      ${flightCard}
