...
function build_steven_artifact_content(array $requestRow, array $contextFiles): array
{
    ...
    $content = [];
    $content[] = 'Steven artifact placeholder';
    $content[] = '';
    $content[] = 'Request Title: ' . $title;
    $content[] = 'Target Path: ' . ($targetPath !== '' ? $targetPath : '[determine target file]');
    $content[] = '';
    $content[] = 'Requested Work:';
    $content[] = $prompt !== '' ? $prompt : '(empty)';
    $content[] = '';
    $content[] = 'Next step: replace this placeholder with real generated code output.';

    return [
        'title' => $targetPath !== '' ? ('Steven Output - ' . $targetPath) : 'Steven Output - Pending Target',
        'target_path' => $targetPath !== '' ? $targetPath : null,
        'artifact_type' => 'code',
        'output_mode' => 'full_drop_in',
        'content' => implode("\n", $content),
        'notes' => 'Initial Steven artifact placeholder generated by Jake orchestration.',
    ];
}
...
try {
    ...
    switch ($action) {
        ...
        case 'jake_think':
            ...
            $summaryLines[] = 'Target Path: ' . ($artifactSeed['target_path'] ?? '[not determined]');
            $summaryLines[] = '';
