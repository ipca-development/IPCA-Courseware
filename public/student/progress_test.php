...
async function startTest(){
    ...
    const j = await safeFetch('/student/api/test_start.php', {
        cohort_id: COHORT_ID,
        lesson_id: LESSON_ID
    });

    if (!j.ok) {
        setStatus('Start failed: ' + (j.error||'Unknown error'));
        return;
    }
...
async function submitAnswer(answer){
    ...
    const j = await safeFetch('/student/api/test_answer.php', {
        test_id: TEST_ID,
    ...
    if (!j.ok) {
        setStatus('Answer failed: ' + (j.error||'Unknown error'));
        return;
    }
