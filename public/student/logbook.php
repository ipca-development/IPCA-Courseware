...
(function(){
  ...
  function cell(row,i,key){
    if(key === 'actions') {
      if(!isStudentRow(row)) {
        return '<td><button class="alogw-status-pill alogw-lock-pill alogw-lock-pill--locked" type="button" data-locked-record title="You are not authorized to edit this record.">&#128274;</button></td>';
      }
      return `<td><div class="alogw-row-actions"><button class="alogw-status-pill alogw-status-pill--accepted alogw-lock-pill" type="button" data-edit="${i}" title="Student-entered record is editable">&#128275;</button><button class="alogw-mini-btn alogw-mini-btn--edit" type="button" data-edit="${i}">Edit</button><form method="post" onsubmit="return confirm('Erase this student-entered record?');"><input type="hidden" name="action" value="delete_entry"><input type="hidden" name="format" value="${esc(currentFormat)}"><input type="hidden" name="entry_id" value="${esc(row.id || '')}"><button class="alogw-mini-btn alogw-mini-btn--delete" type="submit">Erase</button></form></div></td>`;
    }
  ...
  document.getElementById('slTableBody').addEventListener('click', e => {
    if(e.target.closest('[data-locked-record]')) {
      window.alert('You are not authorized to edit this record.');
      return;
    }
