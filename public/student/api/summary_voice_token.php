...
try {
    ...
    $instructions =
        ...
        . "When the student asks for final review, validation, acceptance, progress test, or says the summary still shows Draft, call request_final_review immediately. Do not say accepted, validated, complete, ready for progress test, or move forward unless the request_final_review tool returns canonical_accepted=true.\n"
        . "If the summary appears ready but request_final_review has not run yet, say only: 'I can run the official final review now.' Then call request_final_review.\n"
        . "Do not tell the student you will mark a section complete or move on when they asked for final review. The official final review is the next step.\n"
        . "When the student says they are done or asks for review, call request_final_review if they mean final/official review; otherwise call analyze_summary_draft before responding. Review their actual written summary; do not rewrite it for them.\n"
    ...
    $body = [
        ...
        'session' => [
            ...
            'tools' => [
                ...
                ['type' => 'function', 'name' => 'request_final_review', 'description' => 'Run the deterministic precheck and then the official canonical final review. Use whenever the student asks to validate, finalize, review, move to progress test, or asks why the UI still shows Draft.', 'parameters' => ['type' => 'object', 'properties' => ['student_request' => ['type' => 'string'], 'summary_excerpt' => ['type' => 'string'], 'summary_html' => ['type' => 'string']], 'required' => ['student_request'], 'additionalProperties' => false]],
                ['type' => 'function', 'name' => 'set_current_task', 'description' => 'Update the current writing assignment state after Maya has verbally assigned it.', 'parameters' => ['type' => 'object', 'properties' => ['mode' => ['type' => 'string'], 'task_text' => ['type' => 'string'], 'short_task_label' => ['type' => 'string'], 'section_id' => ['type' => 'string'], 'coach_state' => ['type' => 'string', 'enum' => ['STATE_REFLECT', 'STATE_WAITING_FOR_SUMMARY_WRITE', 'STATE_VERIFYING_SUMMARY', 'STATE_REMEDIATION', 'STATE_ADVANCE_TOPIC']]], 'required' => ['mode', 'task_text'], 'additionalProperties' => false]],
            ],
            'tool_choice' => 'auto',
        ],
    ];
