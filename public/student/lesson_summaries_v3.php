...
cw_header('My Notebook (v3 · Maya Coach)');
?>
<link rel="stylesheet" href="/assets/summary_coach.css">
<style>
.nb-shell{
  max-width:1120px;
  margin:0 auto;
...
/* =====================================================================
 * v3: Maya Summary Coach wiring
 * =====================================================================
 *
 * The coach mounts inside the modal sidebar (#mayaCoachRoot). The coach
 * is recreated each time a different lesson is opened so its server-side
 * session row is correctly scoped. When Maya runs final_review and
 * approves, the API server-side delegates to LessonSummaryService::
 * checkSummary(), and we use the existing in-page helpers
 * (updateLessonAndTocMetadata, setLessonLockedState, ...) to refresh the
 * canonical UI exactly the way the production "Check my Summary" path
 * already does.
 */
window.IPCA_MAYA_API_URL = '/student/api/summary_coach.php';
window.__mayaCoach = null;
window.__mayaCurrentLessonId = null;

function bootMayaForCurrentLesson(lessonId) {
  if (!window.IPCASummaryCoach || !lessonId) return;

  // Same lesson? Just keep the existing coach.
  if (window.__mayaCurrentLessonId === lessonId && window.__mayaCoach) {
    if (typeof window.__mayaCoach.refreshEditor === 'function') {
      window.__mayaCoach.refreshEditor();
    }
    return;
  }

  const root = document.getElementById('mayaCoachRoot');
  if (!root) return;

  // Reset DOM so a fresh coach can rebuild it.
  root.innerHTML = '';
  root.removeAttribute('data-coach-built');
  root.setAttribute('data-lesson-id', String(lessonId));

  const config = {
    apiUrl: window.IPCA_MAYA_API_URL,
    context: 'lesson_summaries',
    lessonId: lessonId,
    cohortId: COHORT_ID,
    summaryId: 0,
    ...
    onCanonicalAccept: function (canonicalCheck /*, mayaResponse */) {
      try {
        if (canonicalCheck && canonicalCheck.ok && canonicalCheck.review_status === 'acceptable') {
          // Reuse the existing production helpers to flip the lesson row,
          // TOC pill, lock state, and modal chrome to their accepted state.
          const html = (modalEditor && modalEditor.innerHTML) || '';
          updateLessonAndTocMetadata(
            lessonId,
            'acceptable',
            html,
            String(canonicalCheck.review_feedback || ''),
            Number(canonicalCheck.student_soft_locked || 1)
          );
          setLessonLockedState(lessonId, true);
          syncModalFromHidden(lessonId);
          updateModalChromeForLesson(lessonId);
          setPanelFeedback(lessonId, String(canonicalCheck.review_feedback || ''));
          setPanelNotes(lessonId);
          setPanelVersionContext(lessonId);
          showBanner('Summary accepted by Maya and the canonical check. Review the feedback on the right.', 'ok');
        } else {
          // Either canonical check returned not acceptable, or the call
          // failed. Fall back to a fresh server-side status fetch so the UI
          // never falsely shows accepted.
          if (typeof refreshSummaryStatusOnly === 'function') {
            refreshSummaryStatusOnly();
          } else if (canonicalCheck && canonicalCheck.review_status) {
            const html = (modalEditor && modalEditor.innerHTML) || '';
            updateLessonAndTocMetadata(
              lessonId,
              String(canonicalCheck.review_status),
              html,
              String(canonicalCheck.review_feedback || ''),
              Number(canonicalCheck.student_soft_locked || 0)
            );
          }
        }
      } catch (e) {
        try { console.warn('[Maya] canonical refresh failed:', e && e.message); } catch (ignored) {}
      }
    }
  };
...
function teardownMayaCoach() {
  // Soft teardown: drop the instance reference. Server-side session row
...
// Hook into the existing open/close lifecycle without breaking any
// production behaviour. We use saved references to the originals so this
// stays additive.
(function () {
  const _origOpenEditor = openEditor;
  openEditor = function (lessonId) {
    _origOpenEditor(lessonId);
    if (activeLessonId === lessonId) {
      bootMayaForCurrentLesson(lessonId);
    }
  };

  const _origCloseEditor = closeEditor;
  closeEditor = function (lessonId, silent) {
    _origCloseEditor(lessonId, silent);
    // After the original close, if the modal is no longer open, drop Maya.
...
// If the modal is already open at script-eval time (e.g. user reload while
// in the editor), boot Maya now.
if (editorModal.classList.contains('open') && activeLessonId !== null) {
  bootMayaForCurrentLesson(activeLessonId);
}
</script>

<!-- v3: Maya Summary Coach module. Loaded after the inline page script so
     window.IPCASummaryCoach is available before any modal open call (the
