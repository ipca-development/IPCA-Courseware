...
try {
    ...
    $result = $kernel->messages->markRead(
        $session,
        (string)($in['conversation_uuid'] ?? ''),
        (int)($in['last_read_seq'] ?? 0)
    );
    CommunicationHttp::json(200, array('ok' => true, 'receipt' => $result));
} catch (CommunicationException $e) {
    CommunicationHttp::fail($e);
} catch (Throwable $e) {
    CommunicationSupport::log('communication.receipts.error', array('error' => 'server_error', 'class' => $e::class, 'message' => $e->getMessage()));
