...
$tokenRow = ot_find_valid_token($pdo, $token, 'set_password');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$tokenRow) {
            throw new RuntimeException('This password setup link is invalid or has expired.');
        }
        ...
        if ($userId <= 0 || $tokenId <= 0) {
            throw new RuntimeException('Invalid onboarding token context.');
        }

        $pdo->beginTransaction();

        $newStatus = (string)($tokenRow['status'] ?? '') === 'pending_activation'
            ? 'active'
            : (string)($tokenRow['status'] ?? 'active');
        ...
        ot_mark_token_used($pdo, $tokenId);
        ot_invalidate_other_tokens($pdo, $userId, 'set_password', $tokenId);

        $pdo->commit();

        $flashType = 'success';
        $flashMessage = 'Your password has been set successfully. You can now sign in.';
        $isSuccess = true;
        $password = '';
        $passwordConfirm = '';
    } catch (Throwable $e) {
...
<title>Set Password · IPCA Academy</title>
...
<div class="access-shell">
    <div class="access-frame">
        ...
        <div class="access-card-wrap">
            <section class="access-card" aria-label="Set password">
                <div class="access-card-head">
                    <div class="access-card-overline">Access Control</div>
                    <h2 class="access-card-title">Set Password</h2>
                    <p class="access-card-subtitle">
                        Create your password to activate access to your IPCA Academy account.
                    </p>
                </div>
                ...
                <div class="access-card-body">
                    <?php if ($flashMessage !== ''): ?>
                        <div class="access-flash <?php echo $flashType === 'success' ? 'access-flash--success' : 'access-flash--error'; ?>">
                            <?php echo sp_h($flashMessage); ?>
                        </div>
                    <?php endif; ?>

                    <?php if ($isSuccess): ?>
                        <div class="access-actions">
                            <a class="access-btn access-btn--primary" href="/login.php">Go to Login</a>
                        </div>
                    <?php elseif (!$tokenRow): ?>
                    ...
                    <?php else: ?>
                        <div class="access-note">
                            Setting password for <strong><?php echo sp_h(ot_user_display_name($tokenRow)); ?></strong>.
                        </div>

                        <form method="post" action="/set_password.php" novalidate>
                            <input type="hidden" name="token" value="<?php echo sp_h($token); ?>">
                            ...
                            <div class="access-field">
                                ...
                                <input
                                    ...
                                    value="<?php echo sp_h($passwordConfirm); ?>"
                                    required
                                    autocomplete="new-password">
                            </div>

                            <div class="access-actions">
                                <button class="access-btn access-btn--primary" type="submit">Activate Account</button>
                                <a class="access-btn access-btn--secondary" href="/login.php">Back to Login</a>
                            </div>
                        </form>
