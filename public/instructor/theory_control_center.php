...
function sanitizeSummaryHtml(html){
    ...
    tpl.innerHTML=html;
    const allowedTags={P:1,BR:1,UL:1,OL:1,LI:1,STRONG:1,B:1,EM:1,I:1,U:1,H1:1,H2:1,H3:1,H4:1,BLOCKQUOTE:1,SPAN:1,DIV:1,MARK:1};
    const walker=document.createTreeWalker(tpl.content,NodeFilter.SHOW_ELEMENT,null);
    const remove=[];
    while(walker.nextNode()){
        const el=walker.currentNode;
        if(!allowedTags[el.tagName]){remove.push(el);continue;}
...
function renderAttemptItems(items){if(!items||!items.length)return '<div class="tcc-modal-muted">No answer-level items found for this attempt.</div>';let html='<div class="tcc-chat-thread">';items.forEach(function(item,idx){const score=formatScoreValue(item.score_points,item.max_points);const q=item.prompt||item.question_text||'Question';const transcript=item.transcript_text||item.answer_text||item.student_answer||'—';const audio=item.audio_url||'';html+='<div class="tcc-chat-bubble ai"><strong>Question '+(idx+1)+':</strong> '+escapeHtml(q)+'</div>';html+='<div class="tcc-chat-bubble student" onclick="openAnswerAudioModal('+jsArg(q)+','+jsArg(transcript)+','+jsArg(audio)+','+jsArg(score)+')">'+escapeHtml(transcript)+'<div class="tcc-chat-score">Score: '+escapeHtml(score)+'</div><span class="tcc-chat-play">▶ Play audio</span></div>';});html+='</div>';return html;}
...
function renderInlineSummary(resp, studentId, lessonId){
    ...
    const summaryKey=cacheSummaryHtml((lesson.lesson_title||'Lesson Summary')+' — Student Summary', summaryHtml);
    ...
    '<div class="tcc-summary-large-btn"><button type="button" class="tcc-detail-btn primary" onclick="openSummaryLargeModalById('+jsArg(summaryKey)+')">Open Larger Summary</button></div>'+
