...
final class CockpitReplayPipeline
{
    ...
    public function build(
        ...
        $warnings = array();
        $sourceMode = (string)($options['replay_source_mode'] ?? 'g3x_only');
        if ($sourceMode === '' || $sourceMode === 'g3x_first') {
            $sourceMode = 'g3x_only';
        }
        if ($sourceMode !== 'g3x_only') {
            throw new RuntimeException('G3X-only replay reconstruction is required. Multi-source reconstruction is disabled.');
        }
        ...
        if (count($g3xPoints) === 0) {
            throw new RuntimeException('G3X CSV required before replay reconstruction.');
        }
        ...
        $warnings[] = 'Replay v2 built in G3X-only mode; phone GPS/AHRS evidence intentionally ignored for primary replay state.';
        ...
        $instrumentKnots = array(
            ...
            'nav_course_deg' => $this->buildG3xScalarKnots($g3xPoints, 'nav_course_deg'),
            ...
            'velocity_e_mps' => $this->buildG3xScalarKnots($g3xPoints, 'velocity_e_mps'),
            'velocity_n_mps' => $this->buildG3xScalarKnots($g3xPoints, 'velocity_n_mps'),
            'velocity_u_mps' => $this->buildG3xScalarKnots($g3xPoints, 'velocity_u_mps'),
            'aoa' => $this->buildG3xScalarKnots($g3xPoints, 'aoa'),
            'aoa_cp' => $this->buildG3xScalarKnots($g3xPoints, 'aoa_cp'),
            ...
            'com1_standby_mhz' => $this->buildG3xScalarKnots($g3xPoints, 'com1_standby_mhz'),
            'com2_mhz' => $this->buildG3xScalarKnots($g3xPoints, 'com2_mhz'),
            'com2_standby_mhz' => $this->buildG3xScalarKnots($g3xPoints, 'com2_standby_mhz'),
            'nav2_mhz' => $this->buildG3xScalarKnots($g3xPoints, 'nav2_mhz'),
            'nav2_standby_mhz' => $this->buildG3xScalarKnots($g3xPoints, 'nav2_standby_mhz'),
        ...
        $engineKnots = array(
            ...
            'fuel_flow_gph' => $this->buildG3xScalarKnots($g3xPoints, 'fuel_flow_gph'),
            'oil_pressure_psi' => $this->buildG3xScalarKnots($g3xPoints, 'oil_pressure_psi'),
            'oil_temp_f' => $this->buildG3xScalarKnots($g3xPoints, 'oil_temp_f'),
            'fuel_pressure_psi' => $this->buildG3xScalarKnots($g3xPoints, 'fuel_pressure_psi'),
            'fuel_qty_gal' => $this->buildG3xScalarKnots($g3xPoints, 'fuel_qty_gal'),
            'volts' => $this->buildG3xScalarKnots($g3xPoints, 'volts'),
        ...
        $recordingDuration = isset($recording['duration_seconds']) && is_numeric($recording['duration_seconds'])
        ...
        $samples = $this->stabilizeAltitudes($samples, $altitudeBaseline, true);
    ...
    private function buildGridSamples(
        ...
        $baroAltitudeCursor = new ReplaySeriesCursor($baroAltitudeKnots);
        ...
        foreach ($gridTimes as $index => $timeS) {
            ...
            $samples[] = array(
                ...
                'nav_course_deg' => $instrumentValues['nav_course_deg'] ?? null,
                ...
                'velocity_e_mps' => $instrumentValues['velocity_e_mps'] ?? null,
                'velocity_n_mps' => $instrumentValues['velocity_n_mps'] ?? null,
                'velocity_u_mps' => $instrumentValues['velocity_u_mps'] ?? null,
                'aoa' => $instrumentValues['aoa'] ?? null,
                'aoa_cp' => $instrumentValues['aoa_cp'] ?? null,
                ...
                'com1_standby_mhz' => $instrumentValues['com1_standby_mhz'] ?? null,
                'com2_mhz' => $instrumentValues['com2_mhz'] ?? null,
                'com2_standby_mhz' => $instrumentValues['com2_standby_mhz'] ?? null,
                'nav2_mhz' => $instrumentValues['nav2_mhz'] ?? null,
                'nav2_standby_mhz' => $instrumentValues['nav2_standby_mhz'] ?? null,
                ...
                'fd_lateral_mode' => $textValues['fd_lateral_mode'] ?? null,
                'fd_vertical_mode' => $textValues['fd_vertical_mode'] ?? null,
                'autopilot_armed_mode' => $textValues['autopilot_armed_mode'] ?? null,
                'com1_name' => $textValues['com1_name'] ?? null,
                'com1_status' => $textValues['com1_status'] ?? null,
                'com1_standby_name' => $textValues['com1_standby_name'] ?? null,
                'com2_name' => $textValues['com2_name'] ?? null,
                'com2_status' => $textValues['com2_status'] ?? null,
                'com2_standby_name' => $textValues['com2_standby_name'] ?? null,
                'nav2_name' => $textValues['nav2_name'] ?? null,
                'nav2_standby_name' => $textValues['nav2_standby_name'] ?? null,
                ...
                'oil_pressure_psi' => $engineValues['oil_pressure_psi'] ?? null,
                'oil_temp_f' => $engineValues['oil_temp_f'] ?? null,
                'fuel_pressure_psi' => $engineValues['fuel_pressure_psi'] ?? null,
                'fuel_qty_gal' => $engineValues['fuel_qty_gal'] ?? null,
                'volts' => $engineValues['volts'] ?? null,
    ...
    private function normalizeG3xPoints(array $g3xSamples): array
    {
        ...
        foreach ($g3xSamples as $sample) {
            ...
            $points[] = array(
                ...
                'velocity_u_mps' => G3XFlightStreamParser::numericValue($row, 'GPS Velocity U (m/sec)', 'GPSVelU'),
                ...
                'fd_lateral_mode' => trim((string)($row['FD Lateral Mode'] ?? ($row['RollM'] ?? ($row['LatMode'] ?? '')))),
     ...
     * @return list<array{t:float,v:float,source?:string}>
     */
    private function buildG3xScalarKnots(array $g3xPoints, string $field): array
    {
        $knots = array();
        foreach ($g3xPoints as $point) {
            $value = $point[$field] ?? null;
            if ($value !== null) {
                $knots[] = array('t' => (float)$point['t'], 'v' => (float)$value, 'source' => 'g3x');
            }
        }
        usort($knots, fn(array $a, array $b): int => $a['t'] <=> $b['t']);
        return $this->mergeScalarKnots($knots);
     ...
     * @return list<array<string,mixed>>
     */
    private function stabilizeAltitudes(array $samples, array $baseline, bool $preserveRawAltitude = false): array
    {
        if (!$samples) {
            return $samples;
        }
        ...
        foreach ($stationarySegments as [$from, $to]) {
            ...
            if (!$preserveRawAltitude && abs($segmentAlt - (float)$baselineFt) > 35.0) {
        ...
        if ($preserveRawAltitude) {
            ...
            foreach ($samples as $i => &$sample) {
                ...
                if ($speed < self::GROUND_SPEED_MAX_KT) {
                ...
                $sample['altitude_quality_reason'] = isset($segmentAltitudeByIndex[$i])
                    ? 'altitude_local_ground_segment_stabilized'
                    : 'altitude_g3x_as_recorded';
                $previousAlt = $alt;
            }
            unset($sample);
