...
final class SafetyOccurrenceIntakeContextService
{
    ...
    public function occurrenceTypes(int $organizationId): array
    {
        $this->config->requireEnabled($organizationId);
        $nodeType = (string)$this->config->get(
            $organizationId,
            'reporter_occurrence_type_node_type',
            'reporter_occurrence_type'
        );
        $stmt = $this->pdo->prepare(
            'SELECT n.id, n.taxonomy_code, n.label, n.description, n.parent_id, n.sort_order,
                    p.taxonomy_code AS parent_code, p.label AS parent_label
             FROM ipca_safety_taxonomy_nodes n
             LEFT JOIN ipca_safety_taxonomy_nodes p
               ON p.id = n.parent_id AND p.organization_id = n.organization_id
             WHERE n.organization_id = ? AND n.node_type = ? AND n.active = 1
             ORDER BY COALESCE(p.sort_order, n.sort_order), n.sort_order, n.label, n.id'
        );
        $stmt->execute(array($organizationId, $nodeType));
        return array_map(static fn(array $row): array => array(
            'id' => (int)$row['id'],
            'code' => (string)$row['taxonomy_code'],
    ...
    public function requireOccurrenceType(int $organizationId, array $input): array
    {
        $nodeType = (string)$this->config->get(
            $organizationId,
            'reporter_occurrence_type_node_type',
        ...
        $nodeId = (int)($input['occurrence_type_id'] ?? $input['occurrence_type_node_id'] ?? 0);
        $code = trim((string)($input['occurrence_type_code'] ?? $input['category'] ?? ''));
        $sql = 'SELECT id, taxonomy_code, label, parent_id
                FROM ipca_safety_taxonomy_nodes
                WHERE organization_id = ? AND node_type = ? AND active = 1 AND ';
        $params = array($organizationId, $nodeType);
        if ($nodeId > 0) {
            $sql .= 'id = ?';
            $params[] = $nodeId;
        } elseif ($code !== '') {
            ...
            throw new SafetyException(
                'occurrence_type_required',
                'Select an active configured occurrence type.',
                400
            );
        }
        ...
        if (!is_array($row)) {
            throw new SafetyException(
                'occurrence_type_unavailable',
                'The selected occurrence type is not active in the Safety Management System.',
                409
            );
        }
        return $row;
