...
final class CoursewareProgressionV2
{
    ...
    public function isFreeStudyWithinCourseEnabled(int $cohortId, ?int $courseId = null): bool
    {
        $policy = $this->resolveEffectivePolicySet($cohortId, $courseId);
        return !empty($policy['allow_free_study_within_course_before_test_completion']);
    }
    ...
    public function isCourseCompletedForProgression(int $userId, int $cohortId, int $courseId): bool
    {
        if ($courseId <= 0) {
            return false;
        }
        ...
        ";
        $lessonStmt = $this->pdo->prepare($lessonSql);
        $lessonStmt->execute([
            ':cohort_id' => $cohortId,
            ':course_id' => $courseId,
        ]);
        ...
        foreach ($lessonIds as $lessonId) {
            if (!$this->lessonHasCanonicalPass($userId, $cohortId, (int)$lessonId)) {
                return false;
            }
        }
...
public function canAccessLessonContent($userId, $cohortId, $lessonId, $courseId, $unlockAfterLessonId)
{
    ...
    if (!isset($courseListCache[$cohortId])) {
        ...
        foreach ($courseListCache[$cohortId] as $idx => $row) {
            $courseIndexCache[$cohortId][(int)$row['id']] = (int)$idx;
        }
    }
    ...
    if (!isset($coursePassCache[$coursePassKey])) {
        ...
        $coursePassCache[$coursePassKey] = $allPassed;
    }

    return (bool)$coursePassCache[$coursePassKey];
}	
    ...
    public function countDeadlineExtensionsForLesson(int $userId, int $cohortId, int $lessonId): int
    {
        $stmt = $this->pdo->prepare("
            ...
            WHERE user_id = :user_id
              AND cohort_id = :cohort_id
              AND lesson_id = :lesson_id
              AND override_type IN ('extension_1', 'extension_2_final')
        ");
        $stmt->execute([
            ':user_id' => $userId,
            ':cohort_id' => $cohortId,
...
public function getCanonicalPassProgressTestAttemptId(int $userId, int $cohortId, int $lessonId): ?int
{
    $row = $this->getLatestCanonicalPassProgressTestRow($userId, $cohortId, $lessonId);
...
public function ensureRequiredActionsForDecision(int $progressTestId, array $decision): array
{
    ...
    foreach ((array)($requiredActionDecision['action_types'] ?? []) as $actionType) {
        ...
        if ($actionType === 'instructor_approval') {
            $actionUrl = $this->buildInternalAppUrl('/instructor/instructor_approval.php?token=' . urlencode($actionToken));
        } else {
            $actionUrl = $this->buildInternalAppUrl('/student/remediation_action.php?token=' . urlencode($actionToken));
        }
