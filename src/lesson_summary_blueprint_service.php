...
final class LessonSummaryBlueprintService
{
    ...
    private function callBlueprintAi(array $sourceData, array $resourceContext): array
    {
        ...
        $payload = [
            ...
            'text' => [
                'format' => [
                    'type' => 'json_schema',
                    'name' => 'lesson_summary_blueprint_v1',
                    'schema' => $schema,
                    'strict' => true,
                ],
            ],
    ...
    private function normalizeBlueprint(array $raw, array $sourceData, array $resourceContext): array
    {
        ...
        foreach (($raw['summary_structure'] ?? []) as $i => $section) {
            ...
            $sections[] = [
                ...
                'required_concepts' => $this->stringList($section['required_concepts'] ?? []),
                'operational_focus' => $this->stringList($section['operational_focus'] ?? []),
                'not_required' => $this->stringList($section['not_required'] ?? []),
                'completion_requirements' => [
                    'requires_why_reasoning' => (bool)($section['completion_requirements']['requires_why_reasoning'] ?? true),
                    'requires_pilot_action' => (bool)($section['completion_requirements']['requires_pilot_action'] ?? true),
                    'minimum_student_bullets' => max(1, (int)($section['completion_requirements']['minimum_student_bullets'] ?? 1)),
                ],
    ...
    private function validateBlueprint(array $blueprint, array $sourceData, array $resourceContext): array
    {
        ...
        foreach (['ideal student summary', 'polished summary', 'canonical bullet text', 'final summary'] as $phrase) {
