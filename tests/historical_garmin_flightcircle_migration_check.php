...
$checks = array(
    ...
    'FlightCircle matching uses active replacement dataset and explainable tail Hobbs keys' =>
        ...
        && str_contains($files['match_service'], 'matchSelectedGarminCsvFiles')
        && str_contains($files['match_service'], 'matchGarminSegmentToFlightCircle')
        && str_contains($files['match_service'], 'tail_hobbs_range_contains_garmin_leg')
    ...
    'unified Garmin import overview supports bulk process and FlightCircle enrichment' =>
        ...
        && str_contains($files['admin_page'], 'garmin-import-date-main')
        && str_contains($files['admin_page'], 'garmin-fc-pill-cell')
        && str_contains($files['admin_page'], 'csv_file_ids')
        && str_contains($files['admin_page'], 'ipca.garminImports.filters')
