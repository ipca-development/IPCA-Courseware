...
require_contains($store, 'dispatch.consentStatus = dispatch.operationalSessionModelVersion', 'operational consent retired', $failures);
require_contains($store, '$0.consents.removeAll { $0.dispatchID == dispatch.id }', 'operational consent evidence removed', $failures);
...
require_contains($store, 'seedPlannedLegsFromScheduledReservation', 'scheduled open seeds plannedLegs', $failures);
require_contains($store, 'reservationSessions:', 'scheduled open accepts reservation siblings', $failures);
require_contains($catalog, 'leg_sequence_number', 'schedule leg sequence decode', $failures);
require_contains($views, 'reservationSessions: siblings', 'schedule open passes reservation siblings', $failures);
require_contains($views, 'operationalIdentity?.legUUID != nil { return nil }', 'schedule consume by leg not shared scheduler', $failures);
...
require_contains($views, 'Hold 2 seconds — then confirm keep engine running', 'transient stop hold UI', $failures);
require_contains($views, 'Confirm you will keep your engine running and depart on your next leg?', 'transient confirm dialog', $failures);
require_contains($views, 'Did you shut down the engine?', 'shutdown confirm dialog', $failures);
require_contains($views, 'FINALIZE WITHOUT AVIONICS OFF', 'stuck-state force finalize', $failures);
require_contains($views, 'leg:\(legUUID.lowercased())', 'log identity prefers leg_uuid', $failures);
require_contains($views, 'HOLD CARD 5 SECONDS TO VOID', 'log void long-press hint', $failures);
require_contains($views, 'guard adminUnlocked else { return }', 'log void restricted to admin mode', $failures);
require_contains($views, 'voidHoldDuration', 'log void hold duration', $failures);
require_contains($store, 'forceFinalizeEngineShutdownAfterCheckIn', 'force finalize API', $failures);
require_contains($store, 'voidFlightLog', 'void log API', $failures);
require_contains($store, 'cancelRemainingPlannedLegsForEarlyShutdown', 'early shutdown cancel helper retained', $failures);
require_absent_marker($views, 'cancelRemainingPlannedLegsForEarlyShutdown', 'engine shutdown does not auto-cancel remaining legs', $failures);
require_contains($views, 'remaining legs stay available after Check-In', 'full stop keeps remaining legs', $failures);
require_contains($views, 'IN-FLIGHT READY - STBY FOR AVIONICS', 'skip manual recorder after dispatch', $failures);
...
require_contains($views, 'ADD RESERVATION', 'local dispatch entry', $failures);
require_contains($views, 'LocalDispatchSheet', 'route-free local Dispatch sheet', $failures);
...
require_contains($views, 'subtitle: "Route-free Operational Session"', 'local Dispatch does not predict legs', $failures);
require_absent_marker($views, 'Single-leg Dispatch only', 'single-leg toggle removed', $failures);
require_contains($views, 'title: "DISPATCH NOW"', 'two-second Dispatch hold button', $failures);
require_contains($views, 'Hold for 2 seconds to confirm', 'two-second Undispatch hold guidance', $failures);
